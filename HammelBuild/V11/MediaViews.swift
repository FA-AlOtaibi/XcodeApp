    ...
    var body: some View {
        ZStack {
            ...
            if let image {
                ...
                Image(systemName: item.isVideo ? "play.fill" : "doc.fill").font(.title).foregroundStyle(.secondary)
            }
        }
        .task {
            if item.isImage {
...
struct MediaDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                if item.isImage || item.isVideo {
                    Button {
                        Task { await model.saveExistingToPhotos(item.url) }
                    } label: {
                ...
                if item.isVideo {
                    NavigationLink {
                        ...
                        Label("تعديل الفيديو", systemImage: "slider.horizontal.3")
                            .frame(maxWidth: .infinity)
                            .padding(14)
                            .background(.thinMaterial, in: RoundedRectangle(cornerRadius: 17))
                    }
                ...
                Button("حذف الملف", role: .destructive) {
                    showDelete = true
                }
                .padding(.top, 4)
            }
        ...
        .alert("إعادة تسمية", isPresented: $showRename) {
            ...
            Button("حفظ") { model.renameLocal(item, to: name) }
    ...
    private func export() {
        ...
        Task {
            do {
                ...
                try await withCheckedThrowingContinuation { (continuation: CheckedContinuation<Void, Error>) in
                    exp.exportAsynchronously {
                        ...
                        case .failed, .cancelled:
                            continuation.resume(throwing: exp.error ?? AppError.message("فشل التصدير"))
                        default:
                            continuation.resume(throwing: AppError.message("تعذر إكمال التصدير"))
                        }
                    }
                }
                ...
                model.show("تعذر حفظ التعديل", .error)
            }
            exporting = false
        }
    }
