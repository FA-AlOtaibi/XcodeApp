    ...
    var body: some View {
        ZStack {
            ...
            if let image {
                ...
                Image(systemName: item.isVideo ? "play.fill" : item.ext == "gif" ? "photo.stack" : "doc.fill")
                    .font(.title).foregroundStyle(.secondary)
            }
        }
        .task {
            if item.isImage || item.ext == "gif" {
                image = UIImage(contentsOfFile: item.url.path)
            } else if item.isVideo {
                image = await thumbnail(item.url)
            }
        }
    }
...
struct MediaDetailView: View {
    ...
    var body: some View {
            ...
            VStack(spacing: 18) {
                ...
                HStack {
                    ...
                    Button { model.toggleFavorite(item) } label: {
                        Image(systemName: model.isFavorite(item) ? "heart.fill" : "heart")
                            .font(.title3).foregroundStyle(model.isFavorite(item) ? .red : .secondary)
                    }
                }
                ...
                HStack(spacing: 10) {
                    ...
                    Button { showRename = true } label: {
                        Label("إعادة تسمية", systemImage: "pencil").frame(maxWidth: .infinity)
                    }.buttonStyle(.bordered)
                }

                if item.isImage || item.isVideo {
                    Button { Task { await model.saveExistingToPhotos(item.url) } } label: {
                ...
                if item.isVideo {
                    NavigationLink { MediaEditorView(sourceURL: item.url, model: model) } label: {
                        Label("قص وتعديل", systemImage: "slider.horizontal.3")
                            .frame(maxWidth: .infinity).padding(14)
                            .background(.thinMaterial, in: RoundedRectangle(cornerRadius: 17))
                    }
                }

                Button("نقل إلى سلة المهملات", role: .destructive) { showDelete = true }
                    .padding(.top, 4)
            }
            .padding(20)
        }
        ...
        .alert("إعادة تسمية", isPresented: $showRename) {
            ...
            Button("حفظ") { model.renameLocal(item, to: name); dismiss() }
            Button("إلغاء", role: .cancel) {}
        }
        .confirmationDialog("نقل هذا الملف إلى سلة المهملات؟", isPresented: $showDelete) {
            Button("نقل إلى السلة", role: .destructive) {
                model.moveToTrash(item)
                dismiss()
            }
        }
        .sheet(isPresented: $showTools) { MediaQuickActionsSheet(model: model, item: item) }
    }
}
            ...
            VStack(spacing: 18) {
                ...
                Button { export() } label: {
                    HStack {
                        ...
                        Label("حفظ نسخة", systemImage: "square.and.arrow.down")
                    }
                    .frame(maxWidth: .infinity).padding(15).foregroundStyle(.white)
                    .background(Color(red: 0.28, green: 0.34, blue: 0.18), in: RoundedRectangle(cornerRadius: 18))
                }.disabled(exporting || end <= start)
            }.padding(20)
        }
        .navigationTitle("تعديل الفيديو")
        .navigationBarTitleDisplayMode(.inline)
        .task {
    ...
    private func export() {
        ...
        Task {
            do {
                ...
                let original = CMTime(seconds: end - start, preferredTimescale: 600)
                let scaled = CMTime(seconds: (end - start) / speed, preferredTimescale: 600)
                composition.scaleTimeRange(CMTimeRange(start: .zero, duration: original), toDuration: scaled)

                guard let exp = AVAssetExportSession(asset: composition, presetName: AVAssetExportPresetHighestQuality) else {
                    throw AppError.message("تعذر تجهيز التعديل")
                }
                let tmp = FileManager.default.temporaryDirectory.appendingPathComponent("edited-\(Int(Date().timeIntervalSince1970)).mp4")
                try? FileManager.default.removeItem(at: tmp)
                exp.outputURL = tmp; exp.outputFileType = .mp4
                await exp.export()
                guard exp.status == .completed else { throw exp.error ?? AppError.message("فشل التصدير") }
                _ = try model.persist(tmp, tmp.lastPathComponent)
                model.refreshStorage()
                model.show("تم حفظ النسخة المعدلة", .success)
