...
struct ContentView: View {
    ...
    private var header: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                ...
                Text("Product imaging, angles & 3D").font(.caption).foregroundStyle(.secondary)
            }
            Spacer()
            ...
            ZStack(alignment: .bottom) {
                ...
                if let source = processor.source {
                    ...
                    VStack(spacing: 15) {
                        ...
                        Text("عزل، تحسين، Depth، زوايا جديدة، ومجسم 3D حقيقي").font(.subheadline).foregroundStyle(.secondary).multilineTextAlignment(.center)
                    }
                    .padding(35)
                }
        ...
        sectionCard(title: "AI Lab", subtitle: "Hugging Face — يستخدم حسابك وحصتك") {
            VStack(spacing: 11) {
                aiButton(icon: "square.3.layers.3d.down.right", title: "Depth Map", subtitle: "Depth Anything V2", action: {
                    guard let image = processor.source else { return }
                    Task { await hf.generateDepth(from: image) }
                })
                aiButton(icon: "camera.rotate", title: "4 زوايا جديدة", subtitle: "Qwen Image Edit — ±45° و ±90°", action: {
                    ...
                    Task { await hf.generateAngles(from: image) }
                })
                aiButton(icon: "cube.fill", title: "إنشاء مجسم 3D", subtitle: "Hunyuan3D 2.1 — GLB بخامات", badge: "GPU", action: {
                    guard let image = processor.transparent ?? processor.source else { return }
                    Task { await hf.generate3D(from: image, textured: true) }
                })
            }
        }
    }
    ...
    private func modelSection(_ url: URL) -> some View {
        sectionCard(title: "3D Asset", subtitle: "ملف Hunyuan3D جاهز للعرض والتصدير") {
            VStack(spacing: 12) {
                ...
                HStack(spacing: 10) {
                    ...
                    Button {
                        guard let image = processor.transparent ?? processor.source else { return }
                        Task { await hf.generate3D(from: image, textured: false) }
                    } label: { Label("نسخة أسرع", systemImage: "bolt").frame(maxWidth: .infinity) }.buttonStyle(SecondaryButtonStyle())
                }
            }
        }
    }
    ...
    private func errorCard(_ text: String) -> some View {
        HStack(alignment: .top, spacing: 11) {
            ...
            Text(text).font(.footnote).foregroundStyle(.white.opacity(0.88))
            Spacer()
        }
        ...
        NavigationStack {
            Form {
                ...
                Section {
                    ...
                    Text("نماذج ZeroGPU قد تتطلب تسجيل دخول وحصة GPU متاحة في حساب Hugging Face.")
                }
            }
...
private struct SecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label.font(.subheadline.bold()).padding(.vertical, 13)
            .background(.white.opacity(configuration.isPressed ? 0.14 : 0.075), in: RoundedRectangle(cornerRadius: 14))
